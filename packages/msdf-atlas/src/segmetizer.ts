...
class PointRange  {
  ...
  closePath(startPoint) {
    ...
    if (this.last && (this.last.x !== startPoint.x || this.last.y !== startPoint.y)) {
...
export const segmentize = (cmds) => {
  ...
  commands.forEach((cmd, index) => {
    ...
    if(type === 'C') {
      ...
      const step = 1 / curveSegments;
      for(let t = step; t <= 1; t += step) {
        const midPoint = pointOnCubicBezier(startPoint.x, startPoint.y, controlPoint1.x, controlPoint1.y, controlPoint2.x, controlPoint2.y, endPoint.x, endPoint.y, t);
        range.add(midPoint);
      }
      return 
    }
